...
#It assumes the DQuadSetupNC file has been run to install a double quadratic version of PyTransport
#It is recommended you restart the kernel before running this file to insure any updates to PyTransDQuadNC are imported

location = "/nethome/ronayne/Documents/PyTransport-master/PyTransport/" # this should be the location of the PyTransport folder folder
...
fields = np.array([0.4,20.0])# we set up a numpy array which contains the values of the fields
nP=PyT.nP()   # the .np function gets the number of parameters needed for the potential -- this can be used as a useful crosscheck
pvalue = np.zeros(nP) 
M=1./2.*pow(10.,-6.0)
pvalue[0]=0.002*M**2.0
#pvalue[0]=0.02*M**2.0
pvalue[1]=0.004*M**2.0
#pvalue[1]=0.03*M**2.0

pvalue[2]=1.0 # we set up numpy array which contains values of the parameters
